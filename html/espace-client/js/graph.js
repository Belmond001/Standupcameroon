document.addEventListener("DOMContentLoaded", function () {
    ...
    const nb_sondage = document.querySelectorAll(".item-second-container").length;

    for (let j = 1; j <= nb_sondage; j++){
        for (let i = 1; i <= nb_question; i++) {
            const containerId = "sondage" + j+"-container" + i;
            const tableId = "sondage" + j+"-datatable" + i;
